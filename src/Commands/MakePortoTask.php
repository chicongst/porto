...
namespace Chicongst\Porto\Commands;

use Illuminate\Console\Command;
use Illuminate\Filesystem\Filesystem;
use Illuminate\Support\Str;

class MakePortoTask extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'porto:task {name}';
    ...
    public function makeClassName()
    {
        ...
        $fqClass = config('config-porto.namespace', 'App\\Containers\\').$ns.'Tasks\\'.$className;

        if (substr($fqClass, -6, 6) !== 'Task') {
            $fqClass .= 'Task';
        }
